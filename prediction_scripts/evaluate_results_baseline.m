function [to_show_mat]=evaluate_results_baseline(combo_test)

baseline_op = [];
best_op = [];
target_op = [];
for i = 1: length(combo_test)
...
for i = 1:num_annt
	...
	cur_targets = target_op(:,i);

	RMSE_1 = sqrt(1/length(cur_output))* sqrt((cur_targets-cur_output)'*(cur_targets-cur_output));
	corr_coef_1_mat = corrcoef(cur_targets,cur_output);
	corr_coef_1 = corr_coef_1_mat(1,2);
