function [combo_test,all_storage] = perform_cv_em_lr_dev(dbn_size,max_iter,save_file)
...
%		[~, ~, test_output] = nntest(dnn, norm_test_feat, target_test); 
		...
		lr_coefs = pinv(norm_train_feat)*target_train;
		train_output = norm_train_feat*lr_coefs;
		dev_output = norm_dev_feat*lr_coefs;
		test_output = norm_test_feat*lr_coefs;
