function [obt_hidden_G,coefs] = determine_coefs(nn_output,annot_lables,dev_nn_output,dev_annot_lables,test_nn_output,test_annot_lables,win_len)
...
% determining coeffs
coefs = zeros(size(annot_lables,2),length(-1*win_len:win_len));

X = zeros(size(nn_output,1),length(-1*win_len:win_len));
for (i = -1*win_len:win_len)
        X(:,i+1+win_len) = circshift(nn_output,i);
end

for annot_id = 1:size(annot_lables,2)
	cur_Y = annot_lables(:,annot_id);
